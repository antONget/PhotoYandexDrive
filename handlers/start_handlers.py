...
from services.yandex_drive import get_list_folders_to_path, get_download_link
from filter.admin_filter import check_super_admin
from utils.utils_keyboard import utils_handler_pagination_and_select_item
from keyboards.start_keyboard import keyboard_preview_folder
import logging
...
async def process_select_action(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if frame:
        cost = frame.cost
        id_frame = frame.id
    link_preview = await get_download_link(file_path=path)
